...
from mutagenesis.variant_dataset import extract_sequences, prepare_data, get_data_loader
...
def get_variant_scores(model, data_loader, device):
    ...
    with torch.no_grad():
        for reference_tensors, alternative_tensors in data_loader:
            ...
            reference_tensors   = reference_tensors.to(device).permute(0, 2, 1)    # (B, 4, 800)
            alternative_tensors = alternative_tensors.to(device).permute(0, 2, 1)  # (B, 4, 800)

            # Append zero m6A channel so the 5-channel model can score seq-only variants
            zeros = torch.zeros(reference_tensors.shape[0], 1, reference_tensors.shape[2],
                                device=device, dtype=reference_tensors.dtype)
            reference_tensors   = torch.cat([reference_tensors,   zeros], dim=1)   # (B, 5, 800)
            alternative_tensors = torch.cat([alternative_tensors, zeros], dim=1)   # (B, 5, 800)
