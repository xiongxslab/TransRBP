...
parser.add_argument('--m6A_binding', type= int, default=1, help="whether to bind m6A track in input, 1 means enabled")
...
def train_val_test(rbp_name):
    ...
    train_loader = DataLoader(dataset.RBPDataset(rbp_name, args.data_root, m6A_binding=args.m6A_binding), batch_size=args.bs, shuffle=True, num_workers=64, persistent_workers=True)
    val_loader = DataLoader(dataset.RBPDataset(rbp_name, args.data_root, mode='val', m6A_binding=args.m6A_binding), batch_size=args.bs, shuffle=False, num_workers=32, persistent_workers=True)
    test_loader = DataLoader(dataset.RBPDataset(rbp_name, args.data_root, mode='test', m6A_binding=args.m6A_binding), batch_size=args.bs, shuffle=False, num_workers=16, persistent_workers=True)

    model = RBPResTransModels.RBPModel(features = 5 if args.m6A_binding==1 else 4)
    ...
    test_model = RBPResTransModels.RBPModel(features = 5 if args.m6A_binding==1 else 4)    
