...
class TransformerEncoder(torch.nn.TransformerEncoder):
    ...
    def forward(self, src, mask = None, src_key_padding_mask = None):
        ...
        all_attn_weights = torch.stack(attn_weight_list)
...
class AttnModule(nn.Module):
    ...
    def forward(self, x):
        ...
        output, attn_weights = self.module(x)
        max_attn_weights = torch.max(attn_weights, dim=0)[0]
        return output, max_attn_weights
