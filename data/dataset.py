...
class RBPDataset(Dataset):
    ...
    def __init__(self, RBP_name, data_root, mode = 'train'):
    ...
    def __getitem__(self, idx):
        ...
        item = start, end, chrom, direction
    ...
    def save_to_hdf5(self, filedir):
        with h5py.File(filedir+'/'+self.RBP_name+'_'+self.mode+'.h5', 'w') as hf:
            for idx in tqdm(range(len(self))):
                ...
                group.create_dataset('seq_onehot', data=seq_onehot.numpy())
                group.create_dataset('bigwig', data=bigwig.numpy())
